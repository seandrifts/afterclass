...
現金券的成本等於面額全額（直接減少收入），實物券的成本只有食材。同樣讓客人開心 50 元：
...
所以獎項結構是：**實物獎為主 + 一個小額現金券墊底 + 免單當大獎**。不設中額現金券（折 50、折 20），它們的成本效率太差，用實物獎取代。

保留「折 10 元」的唯一理由是需要一個成本低又不寒酸的保底選項，且它有使用門檻（滿 100）可以推高客單。

### 3.3 獎項表

**基準：客單 120 元，食材成本率 35%，免單成本率 38%**（這三個參數在後台 › 設定 › 成本試算可調）

售價來自店家實際菜單，成本 = 售價 × 成本率。

| 獎項 | 類型 | 售價/感知價值 | 成本 | 權重 | 機率 | 名目期望 |
|---|---|---|---|---|---|---|
| 免單（下次消費全免，上限 150） | 現金 | 150 | 57 | 3 | 0.30% | 0.17 |
| 飲料一杯 | 實物 | 50 | 18 | 40 | 4.00% | 0.72 |
| 燙青菜一份 | 實物 | 40 | 14 | 60 | 6.00% | 0.84 |
| 滷蛋一顆 | 實物 | 40 | 14 | 200 | 20.00% | 2.80 |
| 加麵 / 加飯 | 實物 | 10 | 4 | 450 | 45.00% | 1.80 |
| 折 10 元（滿 100 可用） | 現金 | 10 | 10 | 247 | 24.70% | 2.47 |
| **合計** | | | | **1000** | **100%** | **8.80** |

### 3.4 核銷率修正（重要）

上表是「假設每張券都被用掉」的數字。實務上券的核銷率遠低於 100%，實際成本要乘上核銷率。

| 獎項 | 名目期望 | 預估核銷率 | 實際期望成本 |
|---|---|---|---|
| 免單 | 0.17 | 90%（獎太大，一定會用） | 0.15 |
| 飲料一杯 | 0.72 | 55% | 0.40 |
| 燙青菜一份 | 0.84 | 50% | 0.42 |
| 滷蛋一顆 | 2.80 | 50% | 1.40 |
| 加麵 / 加飯 | 1.80 | 45% | 0.81 |
...
**實際期望成本 3.92 元 / 客**，佔客單 3.3%。

以每日 80 客計，每月活動成本約 **9,400 元**。建議月上限設 12,000 留緩衝。

**核銷率是估的，上線兩週後要用真實數據取代。**後台 › 報表 › 核銷率分析會算出每個獎項的實際核銷率，再回頭餵給成本試算。這組數字準了之後，整個成本模型才可信。

### 3.5 效益對比

每月成本 9,400 元。回訪頻率如果從每月 4 次提升到 5 次（+25%）：
...
成本 9,400 換 39,600 增額毛利。就算回訪只提升 10%，也還是划算。

**真正要盯的不是成本，是回訪率有沒有動。**如果跑一個月回訪率沒變化，代表問題在別的地方（口味、價格、地點），這時候該停掉活動而不是加碼獎項。

### 3.6 待確認

店家提供的數字（滷蛋 40 / 加麵加飯 10 / 飲料 50 / 燙青菜 40）目前**認定為售價**，成本用 35% 換算。

若這些其實是成本，機率必須大幅下修（滷蛋從 20% 降到 4%），實際成本會變成 5.13 元/客，且體驗明顯變差（九成的人只會抽到加麵或折 10 元）。切換方式：在後台把各獎項的「實際成本」欄位改成原始數字，再依模擬器重新調權重。
...
| 是否可轉讓 | 否 | 綁定 LINE 帳號 |
| 現金券使用門檻 | 折 10 需滿 100 | 保護毛利，同時推高客單 |
